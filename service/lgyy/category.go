...
const dy555CateGoryApi = "https://www.lgyy.tv/vodshow/%s-%s-%s-%s-%s-%s---%d---%s.html"

func (this *LGYY) GetCategory(typeId string, page int) (res model.Category) {
	...
	if class != "" {
		url = fmt.Sprintf(
			dy555CateGoryApi, class, url2.QueryEscape(area), sort, url2.QueryEscape(item), url2.QueryEscape(lang),
			letter, page,
			year,
		)
	} else {

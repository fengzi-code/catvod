...
const cokemvCateGoryApi = "https://cokemv.me/vodshow/%s-%s-%s-%s-%s-%s---%d---%s.html"

func (this *COKEMV) GetCategory(typeId string, page int) (res model.Category) {
	...
	letter := utils.GetBetweenStr(this.Filters, `letter=`, `&`)
	...
	if class != "" {
		url = fmt.Sprintf(
			cokemvCateGoryApi, class, url2.QueryEscape(area), sort, url2.QueryEscape(item), url2.QueryEscape(lang),
			letter, page,
			year,
		)
	} else {
		url = fmt.Sprintf(
			cokemvCateGoryApi, typeId, url2.QueryEscape(area), sort, url2.QueryEscape(item), url2.QueryEscape(lang),
			letter,
			page, year,
		)
	}

...
const dyHomeApi = "https://cokemv.me/label/new.html"
...
func (this *COKEMV) GetHome() (res model.HomeContent) {
	res.VodClass = make([]model.VodClass, 0)
	classJsonFile := global.CokemvStaticDir + "/class.json"
	...
	if global.AppMode == "f" {
		filterJsonFile = global.CokemvStaticDir + "/f_filters.json"
	} else {
		filterJsonFile = global.CokemvStaticDir + "/filters.json"
	}
	...
	if err != nil {
		fmt.Println("cokemv get home html error: ", err)
		return
	}
	list := htmlquery.Find(
